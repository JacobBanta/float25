...
class DepthSensorController:
    ...
    def control_gpio_pins(self):
        ...
        velocity = (self.current_depth - self.prev_depth) / \
            SENSOR_READ_INTERVAL  # m/s
        ...
        if self.mission_state == 'DESCENDING_TO_PAUSE':
            ...
            if abs(depth_error) <= 0.3:  # Within pause zone
                ...
                # Brake if too fast or close
                if abs(avg_velocity) > speed or depth_error < start_stopping:
                    GPIO.output(5, GPIO.HIGH)
            ...
            self.pause_timer += SENSOR_READ_INTERVAL  # ms
            depth_error = target_depth - self.current_depth
            if abs(depth_error) > 0.5:
                self.pause_timer = 0

            if self.pause_timer >= pause_time:
            ...
            if self.depth_stable_count >= 3 / SENSOR_READ_INTERVAL:  # 3 seconds at 100ms intervals
            ...
            if self.current_depth <= 0.1:  # Near surface
                ...
                GPIO.output(5, GPIO.HIGH)
                GPIO.output(6, GPIO.HIGH)      # Neutral
            elif abs(avg_velocity) > speed:  # Control ascent speed
                GPIO.output(5, GPIO.HIGH)
                GPIO.output(6, GPIO.HIGH)      # Neutral/brake
            else:
                GPIO.output(5, GPIO.LOW)
            ...
            GPIO.output(5, GPIO.HIGH)
            GPIO.output(6, GPIO.HIGH)          # Neutral
            self.pause_timer += SENSOR_READ_INTERVAL
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                sleep_time = max(0, SENSOR_READ_INTERVAL - execution_time)
